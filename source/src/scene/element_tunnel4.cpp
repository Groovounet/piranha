...
void CTunnel4::_RenderTunnel (float fTime) const
{
    ...
    glPushMatrix ();
        ...
        for (float i = 0; i < CIRCLES; i += 1.0f)
        {
            ...
            float fDisplacementX = sinf ((float)PI / 180.0f * (fTunnelDeplacement * MOVE_SPEED1) + i * 0.2f) * 0.2f * fTime / TIME;
            float fDisplacementY = cosf ((float)PI / 180.0f * (fTunnelDeplacement * MOVE_SPEED1) + i * 0.2f) * 0.1f * fTime / TIME;
            fDisplacementX -= cosf ((float)PI / 180.0f * (fTunnelDeplacement * MOVE_SPEED2) + i * 0.2f) * 0.2f * fTime / TIME;
            fDisplacementY -= sinf ((float)PI / 180.0f * (fTunnelDeplacement * MOVE_SPEED2) + i * 0.2f) * 0.1f * fTime / TIME;
